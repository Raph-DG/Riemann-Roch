...
def Ideal.quotOfMul (a : R) (I : Ideal R) :
    (R ⧸ a • I) →ₗ[R] (R ⧸ Ideal.span {a}) := sorry
...
lemma Ideal.quotOfMul_surjective {a : R} (I : Ideal R) :
    Function.Surjective (Ideal.quotOfMul a I) := by sorry
...
lemma _root_.WithZero.map'_eq_zero_iff {α : Type u_1} {β : Type u_2}
    [MulOneClass α] [MulOneClass β] (f : α →* β)
    (k : WithZero α) : WithZero.map' f k = 0 ↔ k = 0 := by
        cases k
        all_goals simp

lemma _root_.WithZero.map'_ne_zero_iff_exists {α : Type u_1} {β : Type u_2}
    [MulOneClass α] [MulOneClass β] (f : α →* β)
    (k : WithZero α) : WithZero.map' f k ≠ 0 ↔ ∃ k' : α, k' = k := by
    have := WithZero.map'_eq_zero_iff f k
    have : ¬ (WithZero.map' f) k = 0 ↔ ¬ k = 0 := Iff.symm (Iff.ne (_root_.id (Iff.symm this)))
    simp[this]
    exact WithZero.ne_zero_iff_exists

lemma ordMonoidWithZeroHom_eq_ord [Nontrivial R] (x : nonZeroDivisors R) :
    ordMonoidWithZeroHom R x =
    WithZero.map' (Nat.castAddMonoidHom ℤ).toMultiplicative (Ring.ord R x) := by
  simp[ordMonoidWithZeroHom]
...
theorem _root_.isFiniteLength_quotient_span_singleton [IsNoetherianRing R]
    ...
    IsFiniteLength R (R ⧸ Ideal.span {x}) := by sorry
...
lemma ordFrac_ne_zero {f : K} (hf : f ≠ 0) : ordFrac R f ≠ 0 := by simp_all only [ne_eq, map_eq_zero,
  not_false_eq_true]
end Ring
